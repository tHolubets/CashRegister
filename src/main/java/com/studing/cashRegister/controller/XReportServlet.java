...
/**
 * Servlet X-report creating
 * @author tHolubets
 */
@WebServlet("/openOrders/xReport")
public class XReportServlet extends HttpServlet {
    private ReportService reportService = ReportService.getInstance();
    private static final Logger logger = LoggerFactory.getLogger(XReportServlet.class);

    /**
     * Method to create and display X-report
     * @param request
     * @param response
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Report xReport = null;
        try {
            xReport = reportService.makeReport(false);
